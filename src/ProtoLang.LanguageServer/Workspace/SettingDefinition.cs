...
/// be added without somebody deciding this. See <see cref="ProtoLangSettings.Definitions"/> for the
...
/// <remarks>
/// <para>
/// Carries how to read and write its own value on a <see cref="ProtoLangSettings"/>, so that the key,
...
{
    private readonly Func<ProtoLangSettings, IReadOnlyList<string>> _read;
    private readonly Func<ProtoLangSettings, IReadOnlyList<string>, ProtoLangSettings> _write;

    internal SettingDefinition(
        string key,
        SettingTrust trust,
        string because,
        Func<ProtoLangSettings, IReadOnlyList<string>> read,
        Func<ProtoLangSettings, IReadOnlyList<string>, ProtoLangSettings> write)
    {
        ...
        if (!key.StartsWith(ProtoLangSettings.Section + ".", StringComparison.Ordinal))
        {
            throw new ArgumentException(
                $"'{key}' must be written with the '{ProtoLangSettings.Section}.' prefix, which is how a user sees it.",
                nameof(key));
        }
    ...
    public string Name => Key[(ProtoLangSettings.Section.Length + 1)..];
    ...
    internal IReadOnlyList<string> StatedIn(ProtoLangSettings settings) => _read(settings);
    ...
    internal ProtoLangSettings With(ProtoLangSettings settings, IReadOnlyList<string> values) => _write(settings, values);
}
