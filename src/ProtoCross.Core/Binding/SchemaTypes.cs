...
/// <summary>One type a ProtoCross author may name, under both spellings that reach it.</summary>
/// <remarks>
...
/// ProtoCross author may write: the full name, and the simple one.
/// </summary>
/// <remarks>
/// <para>
...
/// be a message and is ambiguous only against other messages (<c>PC0020</c>). A type position takes
/// messages and enums together, so a name matching one of each is as ambiguous as one matching two
/// enums (<c>PC0074</c>). An enum in front of a dot is ambiguous only against other enums. Collapsing
...
public sealed class SchemaTypes
{
    ...
    /// is as ambiguous as one matching two enums -- which is <c>PC0074</c>, and is not the rule for a
    /// receiver after <c>extend</c> (messages alone, <c>PC0020</c>) nor for an enum in front of a dot
    ...
    /// diagnostic is <c>PC0020</c>, which counts messages and nothing else.
    /// </remarks>
    ...
    /// <see cref="MessagesNamed"/> to tell <c>PC0020</c> from <c>PC0021</c>.
    /// </para>
    /// <para>
