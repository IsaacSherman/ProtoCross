...
public sealed class DescriptorBundle
{
    ...
    /// an enum constant, a message or an enum type is declared in a <c>.proto</c>, and the ProtoCross
    /// source that uses it has no declaration to point at. Go to definition and hover both stop at
    /// that boundary without this, which is where they are most wanted -- most of what a ProtoCross
    ...
    /// </para>
    /// <para>
    /// Null for anything ProtoCross declares, which has no schema declaration by construction;
