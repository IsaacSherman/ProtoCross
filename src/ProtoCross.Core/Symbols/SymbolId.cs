...
/// the declaration it came from. So for anything ProtoCross declares, this is <em>where</em> the name
...
/// because lexing is deterministic. For anything the schema declares, there is no ProtoCross
...
public readonly record struct SymbolId
{
    ...
    /// <summary>The identity of something declared in ProtoCross source.</summary>
    /// <param name="document">
    ...
    /// file name, so two documents called <c>test.pcross</c> in different directories are
    ...
    /// ProtoCross declares, a fully qualified protobuf name for anything the schema declares.
    /// </summary>
    /// <remarks>
    ...
    /// <summary>Reads as <c>Local:buffer.pcross@142</c>, so a failing assertion diagnoses itself.</summary>
