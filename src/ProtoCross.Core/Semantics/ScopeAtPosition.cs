...
/// <c>PC0040</c> -- a method must be called -- so a method name resolves in call position and
...
/// ProtoCross forbids the other shadowing outright -- a <c>var</c> colliding with an enclosing name
/// is <c>PC0029</c> and never enters scope -- so no two entries can ever share a spelling.
/// </para>
/// <para>
/// Distinct from <see cref="ScopeEntry"/>, which is what the binder published: that one carries the
/// ranges a query needs and a caller must not care about, and exists only for names ProtoCross
/// declares. This one is the answer, and covers fields of the receiver as well, which have no
/// declaration in ProtoCross source at all.
/// </para>
/// </remarks>
/// <param name="Symbol">
