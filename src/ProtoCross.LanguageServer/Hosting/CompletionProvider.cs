...
using SymbolKind = ProtoCross.Symbols.SymbolKind;
...
/// <c>protocross.config.xml</c> stops compilation, and being unable to fix an import while it is
...
public sealed class CompletionProvider
{
    ...
    /// upward for a <c>protocross.config.xml</c> and parsing it -- a directory walk and an XML parse,
    ...
    /// <c>protocross.tests.Ou|ter</c> is not a member of <c>protocross.tests</c> -- and a caret there
    ...
    /// refuses everything else with <c>PC0080</c>, whose help says why: a local, a parameter and a
    /// method result always hold a value, so there is nothing to ask about. So the ordinary answer is
    /// wrong here in a way that is invisible to the sweep, which was not told that <c>PC0080</c> is a
    ...
    /// offered, and accepting one wrote <c>(count).count</c> for <c>PC0039</c> -- or, inside a
    /// <c>has</c>, <c>PC0080</c>. Twelve of the twenty names offered at that caret were invalid.
    /// </para>
    /// <para>
    ...
    /// <c>protocross.tests.Outer</c> was one qualified name rather than three. Replacing only the
    /// segment under the caret turns <c>extend proto|lang.tests.Outer</c> into
    /// <c>extend Inner.tests.Outer</c>, which is <c>PC0021</c> -- the same defect
    ...
    /// ever sits around a dot: the newline and the indentation of <c>protocross.</c> then
    /// <c>tests.Outer</c> are between segments of one name, and the name it composes is
    /// <c>protocross.tests.Outer</c>.
    /// </para>
    /// </remarks>
    private sealed record QualifiedName(SchemaSubject Subject, string Prefix, string Suffix)
    {
        ...
        /// <c>PC0074</c> -- so a retained <c>a.</c> that makes it unambiguous had nothing left to keep
        ...
        public IReadOnlyList<CompletionItem> Writable(IReadOnlyList<CompletionItem> offered)
        {
            ...
            foreach (var item in offered)
            {
                if (Fragment(item.Label) is { } fragment && seen.Add(fragment))
                {
                    ...
                    // and writes 'protocross.tests.Mapped' into a range holding one segment, which
    ...
    /// <c>protocross.</c> then <c>tests.Outer</c> -- is legal source that no single-line range can
    ...
    /// lines at all -- and so do comments: <c>protocross. /* receiver */ tests.Outer</c> is one
    ...
    /// but the answer itself, since a receiver without that method produces <c>PC0058</c> the moment
    /// it is accepted.
    /// </para>
    /// <para>
    ...
    /// <c>PC0057</c>. Something could be offered that inserts both, and it would not be one of these:
    ...
    /// <c>PC0021</c> the moment it was accepted. Ambiguity is the receiver question rather than the
    ...
    /// simple name, writing it unqualified is <c>PC0074</c>, so offering it would be offering a name
    ...
    /// produces <c>var local: count = inner;</c> and <c>PC0025</c>, in the position where the author
    ...
    /// no parentheses is <c>PC0037</c>, an unknown name, because <c>BindName</c> never looks at
    ...
    /// <c>status.TOP_LEVEL_STATUS_OK</c> is <c>PC0039</c> however certainly <c>status</c> is a
    ...
    private static IReadOnlyList<CompletionItem> Members(
        ...
        => receiver switch
        {
            ...
            [
                ...
                // 'has other.helper()' is PC0080, and a method result is the example its help gives
