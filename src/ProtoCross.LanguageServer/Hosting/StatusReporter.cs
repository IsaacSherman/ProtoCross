...
/// configuration reads a <c>protocross.config.xml</c>; building a loader stats the directories beside
...
public sealed class StatusReporter
{
    ...
    public ServerStatus Report(DocumentUri? document)
    {
        ...
        // protocross.config.xml off disk, so resolving twice is both wasted work and a way for one
    ...
    private StatusSection Versions() => new(
        ...
        [
            ...
            new StatusFact("compiler", VersionOf(typeof(ProtoCross.Compilation))),
        ...
        if (document is null)
        {
            return new StatusSection(
                ...
                "No document was named, so there is nothing to resolve settings for. Run this again "
                    + "with a `.pcross` file open to see the include paths and policy it compiles under.");
        }
    ...
    private StatusSection Trust()
    {
        ...
        [
            ...
            .. ProtoCrossSettings.Definitions
    ...
    private static string? TrustNote(WorkspaceTrust trust, int withheld) => trust switch
    {
        ...
        WorkspaceTrust.NotReported =>
            "The client did not say whether this workspace is trusted, so every setting is honoured "
                + "wherever it was written. Both ProtoCross extensions report it; a client configured by "
                + "hand may not.",
        _ => null,
    };
    ...
    /// <remarks>
    /// Resolution reads files -- a <c>protocross.config.xml</c> that may be malformed, directories that
