...
/// because a user looking at a ProtoCross buffer whose schema is broken must not see an empty Problems
...
/// </para>
/// <para>
/// <b>PC0003 is replaced, not supplemented, when protoc said anything structured.</b> Its message is
...
public static class CompilationDiagnostics
{
    ...
    /// What to say in place of PC0003's message when the compilation found no protoc to run, or null
    ...
    private static Range RangeIn(string schema, ProtocDiagnostic entry, Dictionary<string, string?> texts)
    {
        ...
        var lines = new ProtoCross.Diagnostics.LineMap(text);
