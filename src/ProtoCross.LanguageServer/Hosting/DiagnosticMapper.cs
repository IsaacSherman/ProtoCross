...
using Diagnostic = ProtoCross.LanguageServer.Protocol.Lsp.Diagnostic;
...
/// template puts <c>PC0001: no proto imports</c> on one line and the message on the next; a client
/// renders the code itself, in a column of its own, so prefixing the title reconstructs the same
/// reading. It earns its place: <c>PC0003</c>'s message is whatever protoc wrote, and without
...
/// </para>
/// <para>
/// <b>Help text is not appended to the message where that can be avoided.</b> Several ProtoCross
/// diagnostics put the actionable instruction in <c>Help</c> -- <c>PC0078</c> tells the user exactly
...
public sealed class DiagnosticMapper(bool relatedInformationSupported)
{
    ...
    public const string Source = "protocross";
    ...
    /// </param>
    public Diagnostic Map(ProtoCross.Diagnostics.Diagnostic diagnostic, string uri, Range? at = null)
    {
