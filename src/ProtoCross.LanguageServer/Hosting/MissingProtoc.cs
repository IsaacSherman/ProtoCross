...
/// <c>PATH</c> or name it in <see cref="ProtoCrossSettings.ProtocPathKey"/>. The command line's rendering
...
public sealed record MissingProtoc(IReadOnlyList<string> RemovedPathEntries)
{
    ...
    public string Describe()
    {
        ...
        var account =
            "protoc could not be found, so no schema a ProtoCross file imports can be read. "
            ...
            + $"{ProtoCrossSettings.ProtocPathKey} to its full path.";
