...
/// <summary>
/// The ProtoCross language server: one process, one client, every open document.
/// </summary>
/// <remarks>
/// <para>
...
public sealed class LanguageServerHost : IDisposable
{
    ...
    /// document's configuration, which reads a <c>protocross.config.xml</c>; it builds a loader, which
    ...
    private Task<object?> Initialize(JsonElement? parameters)
    {
        ...
        return Task.FromResult<object?>(new InitializeResult
        {
            ...
            ServerInfo = new ServerInfo("protocross-server", Version),
        });
    }
    ...
    private async Task Initialized(CancellationToken cancellationToken)
    {
        ...
        _log.Info($"protocross-server {Version} is ready.");
    }
