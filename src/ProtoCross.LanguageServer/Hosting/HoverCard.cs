...
/// divisor, a numeric conversion: each is a choice <c>protocross.config.xml</c> made (spec 10.4) that
...
/// comment is prose the author wrote. The signature goes in a fenced <c>protocross</c> block so that
...
internal static class HoverCard
{
    ...
    /// of a ProtoCross buffer may never have opened.
    /// </para>
    /// <para>
    ...
    /// <summary>The signature line, set apart so a client colours it as ProtoCross.</summary>
    private static string Signature(string text) => $"```protocross\n{text}\n```";
