...
/// sends <c>file:///c%3A/src/x.pcross</c> where Visual Studio sends <c>file:///C:/src/x.pcross</c>,
...
public sealed class DocumentUri : IEquatable<DocumentUri>
{
    ...
    /// <c>C:\src\x.pcross</c> has scheme <c>file</c> as far as <see cref="Uri"/> is concerned -- so
    ...
    /// <remarks>
    /// <para>
    /// A client that percent-encodes the drive colon -- <c>file:///c%3A/src/x.pcross</c>, which is
    /// what VS Code sends -- gives <see cref="Uri"/> nothing that looks like a drive, so it hands back
    /// <c>/c:/src/x.pcross</c> with the URI's own root slash still attached.
    /// <see cref="Path.GetFullPath(string)"/> then reads that as rooted on the current drive and
    /// produces <c>C:\c:\src\x.pcross</c>: a path that exists nowhere, that no file watcher will
