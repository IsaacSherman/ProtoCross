...
/// paths and its own <c>protocross.config.xml</c>. Settings live on the folder rather than in a map
...
public sealed record WorkspaceFolder
{
    ...
    public WorkspaceFolder(DocumentUri uri, string? name = null, ProtoCrossSettings? settings = null)
    {
        ...
        Settings = settings ?? ProtoCrossSettings.None;
        Key = PathIdentity.KeyFor(path);
    }

    /// <inheritdoc cref="WorkspaceFolder(DocumentUri, string?, ProtoCrossSettings?)"/>
    public static WorkspaceFolder FromPath(string path, string? name = null, ProtoCrossSettings? settings = null)
    ...
    public ProtoCrossSettings Settings { get; init; }
