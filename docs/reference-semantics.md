...
**C# is the reference.** Where the three initial targets disagree about an operation, ProtoCross
...
rules were derived from. Where ProtoCross does *not* take C#'s answer, the row says so and the
[Departures](#departures-from-c) section says why.

This file is a reference table, not a normative document. [`ProtoCross_Spec/`](../ProtoCross_Spec/README.md)
...
| **ProtoCross rule** | What ProtoCross guarantees, in every backend, regardless of the consumer's build. |
| **C++ must emulate** | What the C++ backend emits to produce that guarantee, and why the native behavior is not enough. |
| **Config key** | The `protocross.config.xml` element that selects it, or `--` when the behavior is fixed. |
| **Spec** | The normative section. |

A row whose **ProtoCross rule** is marked *Not yet pinned* is a known gap: the behavior is not
...
| Unary `-` of an unsigned value | `-uint` promotes to `long`; `-ulong` is a compile error. | Rejected outright (`PC0052`). ProtoCross has no implicit promotion to fall back on. | Never emitted. | `--` | 10.1, 10.3 |
...
| Shift count at or past the width, or negative | Masks the count to its low 5 bits for `int` and `uint`, and 6 for `long` and `ulong`. The count must be an `int`, so a wider one is narrowed first, which throws under a checked context if it does not fit. | The count's low bits: the count modulo N, the width of the value shifted, whatever the count's type and sign. Emitted with an explicit mask, before any narrowing: `x << (int)(count & (N-1))`. | **Undefined behavior** for a negative count and for one not less than the width of the promoted left operand. Emitted as `x << (count & (N-1))`, which is always in range. | `--` | 10.1 |
| Signed `<<` into or past the sign bit | Keeps the low bits, two's complement. Unaffected by a checked context. | Keeps the low N bits under every overflow policy: `Checked` does not terminate and `Saturating` does not clamp. | Defined since C++20, as the value congruent to `x * 2^count` modulo 2^N; undefined for a negative value before it. A bare `<<` with the masked count. | `--` | 10.1 |
| Signed `>>` | Arithmetic: copies the sign bit in, rounding toward negative infinity. | The same, so `-7 >> 1` is `-4` where `-7 / 2` is `-3`. Unsigned `>>` fills with zeroes. | Arithmetic since C++20; implementation-defined before it. A bare `>>` with the masked count. | `--` | 10.1 |
| `&` `\|` `^` `~` | Bitwise on integers. `&`, `\|` and `^` on `bool` are logical operators that do not short-circuit. | Integers only, and two of one type for a binary operator (`PC0085`, `PC0086`, `PC0048`). Governed by no overflow policy, since none can leave its type's range. | Matches natively. No integer type is narrower than `int`, so nothing is promoted. | `--` | 9.2 |
...
guess in [#20](https://github.com/IsaacSherman/ProtoCross/issues/20), which read "C# standard
...
ProtoCross has **no implicit numeric conversions** (a departure -- see below). Every conversion below
...
| Integer literal typing | A literal takes its type from context, with suffixes available; unsuffixed and uncontexted, it is the first of `int`, `uint`, `long`, `ulong` that holds it. | A literal adopts the expected type at its use site when the value fits, so `var total: int64 = 0;` needs no suffix, and there are none. Where nothing is expected it is `int64`, or `uint64` where only `uint64` holds it. The operand of a conversion carries no expectation into itself, so `3000000000 as int32` is a narrowing conversion that wraps rather than a literal reported as out of range. | Emitted with an explicit suffix. | `--` | 6.6, 10.3 |
...
| Area | C# behavior | ProtoCross rule | C++ must emulate | Config key | Spec |
|---|---|---|---|---|---|
| Singular message field, unset -- read through | `NullReferenceException`. | **Compile error (`PC0078`)** unless presence has been established. See [Departures](#departures-from-c). | Nothing: the situation is unrepresentable, so a guarded read emits the plain accessor chain in both targets. | `Presence/UnsetMessageRead` | 13.1 |
| Singular message field, unset -- used as a value | Propagates `null` into the local, argument, or receiver. | `PC0078`. The rule is about the *value*, not only about reading through it: assigning it to a local or passing it as an argument launders exactly the same divergence. | Would otherwise propagate the default instance. | `Presence/UnsetMessageRead` | 13.1 |
...
| proto3 implicit-presence scalar -- presence test | No `HasFoo` is generated. | `PC0079`. The diagnostic exists because the question has no answer on the wire, not because it is hard to implement. | No `has_foo()` is generated. | `--` | 8.4 |
...
| Repeated field -- presence test | No `HasFoo`; repeated fields have no presence. | `PC0079`. | No `has_foo()`. | `--` | 14.1 |
...
unrepresentable. ProtoCross takes the second, for the same reason `on_zero` exists -- the dangerous
case should not be writable by accident. So ProtoCross keeps C#'s *semantic* position, that an unset
...
1. Write the normative rule in the appropriate `ProtoCross_Spec/§N-*.md` file first, and record the
   decision in `ProtoCross_Spec/§31-Decision Log.md`.
...
5. If the behavior is selectable, add the key to `protocross.config.xml` and name it in the
