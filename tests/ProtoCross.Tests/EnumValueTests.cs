...
public class EnumValueTests
{
    ...
    public void ResolvesAValueOfATopLevelEnumBySimpleName()
    {
        ...
        Assert.Equal("protocross.tests.TopLevelStatus", value.EnumType.Descriptor.FullName);
    }
    ...
    public void ResolvesAValueOfANestedEnumBySimpleName()
    {
        ...
        Assert.Equal("protocross.tests.Outer.Nested", SingleEnumValue(result, "f").EnumType.Descriptor.FullName);
    }
    ...
    public void ResolvesAValueByFullyQualifiedEnumName()
    {
        var result = Compile(
            ...
            """
            extend Outer {
                fn f() -> protocross.tests.Outer.Inner.Deep {
                    return protocross.tests.Outer.Inner.Deep.DEEP_NONE;
                }
            }
            """);
    ...
    public void RejectsAValueTheEnumDoesNotDeclare()
    {
        ...
        var diagnostic = Assert.Single(result.Diagnostics, d => d.Code == "PC0076");
        Assert.Contains("NOPE", diagnostic.ToString(), StringComparison.Ordinal);
        Assert.Contains("protocross.tests.TopLevelStatus", diagnostic.ToString(), StringComparison.Ordinal);
    }
    ...
    public void RejectsAValueNamedThroughAnAmbiguousSimpleName()
    {
        ...
        var diagnostic = Assert.Single(result.Diagnostics, d => d.Code == "PC0074");
        Assert.Contains("protocross.tests.ambiguous.First.Kind", diagnostic.ToString(), StringComparison.Ordinal);
        Assert.Contains("protocross.tests.ambiguous.Second.Kind", diagnostic.ToString(), StringComparison.Ordinal);
    }
    ...
    public void ReportsAnUnknownReceiverNameOnlyOnce()
    {
        ...
        Assert.Single(result.Diagnostics, d => d.Code == "PC0037");
        Assert.DoesNotContain(result.Diagnostics, d => d.Code == "PC0076");
    }
