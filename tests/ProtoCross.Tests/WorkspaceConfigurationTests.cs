...
public class WorkspaceConfigurationTests
{
    ...
    {
        var directory = Path.Combine(Path.GetTempPath(), "protocross-" + label, Guid.NewGuid().ToString("N"));
    ...
    private static ProtoCrossSettings Read(out DiagnosticBag diagnostics, params SettingValue[] values)
    {
        diagnostics = new DiagnosticBag();
        return ProtoCrossSettings.Read(values, ConfigurationSource.WorkspaceSetting, diagnostics);
    }
    ...
    private static DocumentUri Document(string directory, string name = "source.pcross")
    ...
    {
        var path = Path.Combine(TempDirectory(), "source.pcross");
    ...
    public void APercentEncodedSpaceDoesNotMakeASecondDocument()
    {
        ...
        var path = Path.Combine(directory, "source.pcross");
    ...
    public void ARedundantSegmentDoesNotMakeASecondDocument()
    {
        ...
        var straight = Path.Combine(directory, "source.pcross");
        var roundabout = Path.Combine(directory, ".", "source.pcross");
    ...
    public void APercentEncodedDriveColonNamesTheSameFileAsAPlainOne()
    {
        ...
        var plain = DocumentUri.Parse("file:///C:/src/app/source.pcross");
        var encoded = DocumentUri.Parse("file:///C%3A/src/app/source.pcross");
    ...
    public void AForwardSlashedWindowsPathIsTheSameDocumentAsABackslashedOne()
    {
        ...
        Assert.Equal(
            DocumentUri.FromPath(@"C:\src\app\source.pcross"),
            DocumentUri.FromPath("C:/src/app/source.pcross"));
    }
    ...
    public void AUncFileUriAndAUncPathAreOneDocument()
    {
        ...
        Assert.Equal(
            DocumentUri.FromPath(@"\\server\share\source.pcross"),
            DocumentUri.Parse("file://server/share/source.pcross"));
    }

    [Fact]
    public void ADocumentKeepsTheUriTheClientSent()
    {
        const string sent = "file:///C%3A/src/app/source.pcross";
    ...
    {
        Assert.False(DocumentUri.TryParse("source.pcross", out _));
    ...
    public void TwoDocumentsInDifferentFoldersResolveDifferentFolderSettings()
    {
        ...
        var workspace = Workspace(
            WorkspaceFolder.FromPath(
                first,
                settings: new ProtoCrossSettings { ProtocPath = firstProtoc, IncludePaths = ["schemas"] }),
            WorkspaceFolder.FromPath(
                second,
                settings: new ProtoCrossSettings { ProtocPath = secondProtoc, IncludePaths = ["schemas"] }));
    ...
    public void AFolderSettingWinsOverAWorkspaceSetting()
    {
        ...
        var folder = WorkspaceFolder.FromPath(directory, settings: new ProtoCrossSettings { ProtocPath = wanted });

        var resolved = Workspace(folder)
            .WithWorkspaceSettings(new ProtoCrossSettings { ProtocPath = TempFile(directory, "other-protoc") })
    ...
    public void AWorkspaceSettingWinsOverAUserSetting()
    {
        ...
        var resolved = Workspace(WorkspaceFolder.FromPath(directory))
            .WithWorkspaceSettings(new ProtoCrossSettings { ProtocPath = wanted })
            .WithUserSettings(new ProtoCrossSettings { ProtocPath = TempFile(directory, "other-protoc") })
            .Resolve(Document(directory));
    ...
    public void AnEditorSettingWinsOverTheEnvironmentVariable()
    {
        ...
        var resolved = (Workspace(WorkspaceFolder.FromPath(directory)) with
        {
            ...
            .WithUserSettings(new ProtoCrossSettings { ProtocPath = wanted })
            .Resolve(Document(directory));
    ...
    public void AProtocThatDoesNotExistIsReportedAndTheNextSourceIsUsed()
    {
        ...
        var resolved = (Workspace(WorkspaceFolder.FromPath(directory)) with
        {
            ...
            .WithUserSettings(new ProtoCrossSettings { ProtocPath = Path.Combine(directory, "missing-protoc") })
            .Resolve(Document(directory));

        Assert.Equal(real, resolved.ProtocPath);
        Assert.Equal(ConfigurationSource.Environment, resolved.ProtocPathSource);
        Assert.Contains(resolved.Diagnostics, diagnostic => diagnostic.Code == "PC2105");
    }
    ...
    public void TheWholePrecedenceStackResolvesConsistently()
    {
        ...
        var workspaceConfig = TempFile(directory, "workspace-policy.xml", "<ProtoCross></ProtoCross>");
        var userConfig = TempFile(directory, "user-policy.xml", "<ProtoCross></ProtoCross>");

        var folder = WorkspaceFolder.FromPath(
            directory,
            settings: new ProtoCrossSettings
            {
    ...
    public void IncludePathsFromEveryScopeAreSearchedMostSpecificFirst()
    {
        ...
        var folder = WorkspaceFolder.FromPath(directory, settings: new ProtoCrossSettings { IncludePaths = [folderRoot] });

        var resolved = Workspace(folder)
            .WithUserSettings(new ProtoCrossSettings { IncludePaths = [userRoot] })
            .Resolve(Document(directory));
    ...
    public void OneDirectoryNamedAtTwoScopesIsSearchedOnce()
    {
        ...
        var folder = WorkspaceFolder.FromPath(directory, settings: new ProtoCrossSettings { IncludePaths = [shared] });

        var resolved = Workspace(folder)
            .WithUserSettings(new ProtoCrossSettings { IncludePaths = [shared + Path.DirectorySeparatorChar] })
            .Resolve(Document(directory));
    ...
    public void ARelativeIncludePathResolvesAgainstTheFolderThatSuppliedIt()
    {
        ...
        var folder = WorkspaceFolder.FromPath(directory, settings: new ProtoCrossSettings { IncludePaths = ["schemas"] });
    ...
    public void ARelativeIncludePathAtUserScopeIsRefusedAndSaidSo()
    {
        ...
        var resolved = Workspace(WorkspaceFolder.FromPath(directory))
            .WithUserSettings(new ProtoCrossSettings { IncludePaths = ["schemas"] })
            .Resolve(Document(directory));
    ...
    public void ARelativeWorkspaceSettingResolvesAgainstTheOnlyOpenFolder()
    {
        ...
        var resolved = Workspace(WorkspaceFolder.FromPath(directory))
            .WithWorkspaceSettings(new ProtoCrossSettings { IncludePaths = ["schemas"] })
            .Resolve(Document(directory));
    ...
    public void ARelativeWorkspaceSettingResolvesAgainstTheWorkspaceFileDirectory()
    {
        ...
        var resolved = Workspace(WorkspaceFolder.FromPath(first), WorkspaceFolder.FromPath(second))
            .WithWorkspaceSettings(new ProtoCrossSettings { IncludePaths = ["schemas"] }, workspaceDirectory)
    ...
    public void ARelativeWorkspaceSettingIsRefusedWhenSeveralFoldersAreOpenAndNoWorkspaceFileSaysWhere()
    {
        ...
        Assert.Empty(resolved.IncludePaths);
        Assert.Contains(resolved.Diagnostics, diagnostic => diagnostic.Code == "PC2103");
    }
    ...
    public void AnEmptyIncludePathListDoesNotHideLowerPriorityIncludePaths()
    {
        ...
        var folder = WorkspaceFolder.FromPath(directory, settings: new ProtoCrossSettings { IncludePaths = [] });
    ...
    {
        var settings = Read(out var diagnostics, new SettingValue("protocross.overflow", "Checked"));

        Assert.True(settings.StatesNothing);

        var refusal = Assert.Single(diagnostics);
        Assert.Equal("PC2101", refusal.Code);
        Assert.Equal(DiagnosticSeverity.Warning, refusal.Severity);
        Assert.Contains(ProjectConfig.FileName, refusal.Message);
        Assert.Contains(ProtoCrossSettings.ConfigPathKey, refusal.Help!);
    }
    ...
    public void EverySettingTheConfigFileOwnsIsRefusedByName()
    {
        foreach (var key in ProjectConfig.Keys)
        {
            ...
            Read(out var diagnostics, new SettingValue($"protocross.{leaf}", "whatever"));

            Assert.Equal("PC2101", Assert.Single(diagnostics).Code);
        }
    }
    ...
    {
        Read(out var diagnostics, new SettingValue("protocross.protokPath", "protoc"));

        var refusal = Assert.Single(diagnostics);
        Assert.Equal("PC2102", refusal.Code);
        Assert.Contains(ProtoCrossSettings.ProtocPathKey, refusal.Help!);
    }

    [Fact]
    public void AQualifiedKeyAndABareOneAreTheSameSetting()
    {
        var qualified = Read(out var first, new SettingValue("protocross.protocPath", "protoc"));
    ...
    {
        var cleared = Read(out var diagnostics, new SettingValue("protocross.protocPath", "   "));
    ...
    {
        var settings = new ProtoCrossSettings { ProtocPath = "", ConfigPath = "  ", IncludePaths = ["", " "] };
    ...
    public void ARefusalNamesTheScopeItWasWrittenAt()
    {
        ...
        ProtoCrossSettings.Read(
            [new SettingValue("protocross.nonsense", "x")],
            ConfigurationSource.FolderSetting,
            diagnostics);
    ...
    {
        var directory = TempDirectory();
        TempFile(directory, ProjectConfig.FileName, "<ProtoCross></ProtoCross>");
        var elsewhere = TempFile(TempDirectory(), "policy.xml", CheckedOverflow);

        var resolved = Workspace(WorkspaceFolder.FromPath(directory))
            .WithWorkspaceSettings(new ProtoCrossSettings { ConfigPath = elsewhere })
            .Resolve(Document(directory));
    ...
    public void ARelativeConfigPathResolvesAgainstTheFolderThatSuppliedIt()
    {
        ...
        var folder = WorkspaceFolder.FromPath(directory, settings: new ProtoCrossSettings { ConfigPath = "policy.xml" });
    ...
    public void ARelativeConfigPathAtWorkspaceScopeResolvesAgainstTheWorkspaceFileDirectory()
    {
        ...
        var resolved = Workspace(WorkspaceFolder.FromPath(first), WorkspaceFolder.FromPath(second))
            .WithWorkspaceSettings(new ProtoCrossSettings { ConfigPath = "policy.xml" }, workspaceDirectory)
            .Resolve(Document(first));
    ...
    public void AConfigPathThatNamesNothingIsReportedAndTheSearchHappensAnyway()
    {
        ...
        var resolved = Workspace(WorkspaceFolder.FromPath(directory))
            .WithWorkspaceSettings(new ProtoCrossSettings { ConfigPath = Path.Combine(directory, "gone.xml") })
            .Resolve(Document(directory));

        Assert.Equal("PC2104", Assert.Single(resolved.Diagnostics).Code);
    ...
    public void ARefusedConfigFileIsNamedAndSaysWhatItCosts()
    {
        ...
        var refusal = Assert.Single(resolved.Diagnostics, diagnostic => diagnostic.Code == "PC2106");
        ...
        Assert.Contains("PC2002", refusal.Message);
    ...
    public void ARefusedConfigFileNamedByASettingSaysWhichSettingChoseIt()
    {
        ...
        var resolved = Workspace(WorkspaceFolder.FromPath(directory))
            .WithWorkspaceSettings(new ProtoCrossSettings { ConfigPath = path })
            .Resolve(Document(directory));

        Assert.True(resolved.ConfigRefused);

        var refusal = Assert.Single(resolved.Diagnostics, diagnostic => diagnostic.Code == "PC2106");
        Assert.Contains(ProtoCrossSettings.ConfigPathKey, refusal.Message);
    ...
    public void AMissingConfigFileIsNotARefusal()
    {
        ...
        Assert.DoesNotContain(resolved.Diagnostics, diagnostic => diagnostic.Code == "PC2106");
    }
    ...
    public void ACompilationCannotBeBuiltWithoutTheProtocThatWasResolved()
    {
        ...
        var folder = WorkspaceFolder.FromPath(directory, settings: new ProtoCrossSettings { ProtocPath = protoc });
    ...
    public void EveryResolvedValueSaysWhereItCameFrom()
    {
        ...
        var folder = WorkspaceFolder.FromPath(
            directory,
            settings: new ProtoCrossSettings { ProtocPath = protoc, IncludePaths = ["schemas"] });
    ...
    public void ChangingASettingAdvancesTheGeneration()
    {
        ...
        var second = first.WithUserSettings(new ProtoCrossSettings { ProtocPath = "protoc" });
    ...
    public void AChangedSettingTakesEffectOnTheNextResolutionWithNothingRestarted()
    {
        ...
        var before = Workspace(WorkspaceFolder.FromPath(directory))
            .WithWorkspaceSettings(new ProtoCrossSettings { IncludePaths = [first] });
        var after = before.WithWorkspaceSettings(new ProtoCrossSettings { IncludePaths = [second] });
    ...
    public void ResolvedIncludePathsUseTheSameIdentityTheDescriptorCacheUses()
    {
        ...
        var folder = WorkspaceFolder.FromPath(directory, settings: new ProtoCrossSettings { IncludePaths = [sameSchemas] });
        var resolved = Workspace(folder).Resolve(Document(directory));
    ...
    public void ResolvingOnlyTheImportRootsSettlesThemExactlyAsResolvingEverythingDoes()
    {
        ...
        var settings = Read(
            out _,
            new SettingValue(ProtoCrossSettings.ProtocPathKey, protoc),
            new SettingValue(ProtoCrossSettings.IncludePathsKey, schemas));
    ...
    /// keystroke, and searching upward for a <c>protocross.config.xml</c> and parsing it is a directory
    ...
    public void ResolvingOnlyTheImportRootsNeverConsultsTheConfigurationFile()
    {
        ...
        var settings = Read(out _, new SettingValue(ProtoCrossSettings.IncludePathsKey, schemas));
        var configuration = Workspace(WorkspaceFolder.FromPath(directory) with { Settings = settings });
        var document = Document(directory);
