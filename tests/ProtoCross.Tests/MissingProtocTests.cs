...
public class MissingProtocTests
{
    ...
    {
        TextDocument = new TextDocumentItem { Uri = uri, LanguageId = "protocross", Version = 1, Text = Importing },
    };

    private static string DocumentIn(string directory)
        => new Uri(Path.Combine(directory, "source.pcross")).AbsoluteUri;
    ...
    public void TheAccountSaysWhereToGetProtocAndHowToPointAtIt()
    {
        ...
        Assert.Contains(ProtoCrossSettings.ProtocPathKey, account, StringComparison.Ordinal);
    ...
    public async Task ADocumentThatFoundNoProtocSaysSoWithTheSameAccount()
    {
        ...
        var missing = Assert.Single(published.Diagnostics, diagnostic => diagnostic.Code == "PC0003");
