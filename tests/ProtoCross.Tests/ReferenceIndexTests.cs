...
public class ReferenceIndexTests
{
    ...
    /// example an enum named <c>protocross.conformance.EnumCase.Level</c>, where the whole name is
    ...
    public void AnExtendBlockIsAReferenceToTheMessageItExtends()
    {
        ...
        Assert.Equal("protocross.tests.Outer", reference.Symbol.Key);
    }
    ...
    public void ADeclaredVariableTypeIsAReferenceToTheTypeItNames()
    {
        ...
        Assert.Equal("protocross.tests.TopLevelStatus", reference.Symbol.Key);
    }
    ...
    public void ACastTargetThatNamesAMessageIsStillAReferenceToIt()
    {
        ...
        Assert.Equal(SymbolKind.MessageType, reference.Symbol.Kind);
        Assert.Equal("protocross.tests.Outer.Inner", reference.Symbol.Key);
    }
    ...
    public void AnEnumValueAndTheTypeQualifyingItAreTwoReferences()
    {
        ...
        Assert.Equal("protocross.tests.TopLevelStatus.TOP_LEVEL_STATUS_OK", value.Symbol.Key);
    }
    ...
    public void APresenceTestIsAReferenceToTheFieldItAsksAbout()
    {
        ...
        Assert.Equal("protocross.tests.Outer.optional_count", reference.Symbol.Key);
    }
    ...
    public void AFieldThatCannotAnswerAPresenceTestIsStillReferencedByOne()
    {
        ...
        // PC0079 either way: a repeated field has no presence, and neither does a proto3 string
        // that was not declared optional.
        Assert.Equal(2, result.Diagnostics.Count(diagnostic => diagnostic.Code == "PC0079"));

        var repeated = At(model, source, "has items", "items");
        var implicitly = At(model, source, "has name", "name");

        Assert.Equal("protocross.examples.Invoice.items", repeated.Symbol.Key);
        Assert.Equal("items".Length, repeated.Span.Length);
        Assert.Equal("protocross.examples.InvoiceItem.name", implicitly.Symbol.Key);
    ...
    public void AnArgumentSuppliedTwiceIsReferencedTwice()
    {
        ...
        Assert.Contains(result.Diagnostics, diagnostic => diagnostic.Code == "PC0065");
    ...
    public void AQualifiedTestReceiverIsOneReferenceOverItsWholeName()
    {
        ...
        var qualified = "protocross.examples.InvoiceItem";
        var receiver = At(model, source, "test " + qualified, qualified);
        var method = At(model, source, qualified + ".f \"", "f \"");

        Assert.Equal("protocross.examples.InvoiceItem", receiver.Symbol.Key);
    ...
    public void AnArgumentNamingADuplicatedParameterAnswersWithOneSymbol()
    {
        ...
        Assert.Contains(result.Diagnostics, diagnostic => diagnostic.Code == "PC0026");
    ...
    public void ATestTargetNamingNoMethodStillReferencesItsReceiver()
    {
        ...
        Assert.Contains(result.Diagnostics, diagnostic => diagnostic.Code == "PC0058");
    ...
    /// declaration is in a .proto ProtoCross does not own. #41 is what will answer with a location
    /// there.
    /// </summary>
    [Fact]
    ...
    /// back the same symbol, and a ProtoCross symbol reaches the declaration #39 records for it.
    /// </summary>
    [Fact]
    ...
    public void TheSameTextIndexesIdentically()
    {
        var identity = SourceIdentity.FromPath(
            Path.Combine(TestPaths.CreateTempDirectory(), "buffer.pcross"));
    ...
    public void ABufferThatDoesNotParseStillIndexesWhatBound()
    {
        ...
        Assert.Equal("protocross.examples.Invoice.items", reference.Symbol.Key);
    ...
    public void AnUnknownSymbolHasNoReferencesAndNoDeclaration()
    {
        var stranger = SymbolId.ForDeclaration(
            SymbolKind.Local,
            SourceIdentity.Unsaved("nowhere.pcross"),
            SourceSpan.None);
    ...
    /// ProtoCross declares, and the last segment of the protobuf full name for anything the schema
    /// declares.
    /// </summary>
