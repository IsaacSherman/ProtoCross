...
{
    private const string Name = "test.pcross";
    ...
    {
        var first = SourceSpan.SingleLine("a.pcross", 0, 1, 1, 1);
        var second = SourceSpan.SingleLine("a.pcross", 4, 1, 5, 1);

        Assert.Equal("a.pcross", SourceSpan.Union(first, second).File);
        Assert.Equal("relabelled.pcross", SourceSpan.Union("relabelled.pcross", first, second).File);
    }
    ...
    {
        var here = SourceSpan.SingleLine("a.pcross", 0, 1, 1, 1);
        var elsewhere = SourceSpan.SingleLine("b.pcross", 400, 40, 1, 1);
    ...
    {
        Assert.Equal("test.pcross:3:7", SourceSpan.SingleLine(Name, 20, 3, 7, 4).ToString());
    ...
    public void ADiagnosticAgainstAMultiLineDeclarationCoversAllOfIt()
    {
        ...
        var span = Assert.Single(result.Diagnostics, d => d.Code == "PC0022").Span;
