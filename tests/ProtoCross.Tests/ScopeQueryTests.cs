...
public class ScopeQueryTests
{
    ...
    public void ALocalIsNotVisibleInsideItsOwnInitializer()
    {
        ...
        Assert.Contains(result.Diagnostics, diagnostic => diagnostic.Code == "PC0037");
    }
    ...
    /// <summary>
    /// ProtoCross forbids shadowing outright: an inner <c>var</c> of a name already in scope is
    /// PC0029 and never enters, so the *outer* one keeps binding inside the inner block. A walk of
    ...
    public void AShadowingDeclarationIsRefusedAndTheOuterNameKeepsBinding()
    {
        ...
        Assert.Contains(result.Diagnostics, diagnostic => diagnostic.Code == "PC0029");
    ...
    /// <summary>
    /// A signature can hold two parameters of one name -- PC0026, and still bound. Only the first
    ...
    /// </para>
    /// <para>
    /// PC0078 is expected and allowed. A message field with no established presence is a name that
    ...
    /// one in front of it is not ProtoCross.
    /// </remarks>
