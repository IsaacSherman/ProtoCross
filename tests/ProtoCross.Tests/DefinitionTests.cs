...
/// <summary>
/// Where the name under the caret was declared: in this buffer for what ProtoCross declares, and in
...
public class DefinitionTests
{
    private const string Source =
        ...
        extend protocross.tests.Outer {
            ...
            fn picked() -> protocross.tests.TopLevelStatus {
                var chosen: protocross.tests.TopLevelStatus = TopLevelStatus.TOP_LEVEL_STATUS_OK;
    ...
    public async Task ATestTargetJumpsToTheMethodUnderTest()
        => Assert.Equal(
            Declares(Source, "scaled(scale: int64)", "scaled"),
            SelectionOf(await OneAsync("test protocross.tests.Outer.scal"), "scaled"));
        ...
        foreach (var marker in new[]
                 {
                     ...
                     "extend protocross.tests.Out", "TopLevelStatus.TOP_LEVEL_STATUS_O",
                 })
        {
    ...
    {
        var link = await OneAsync("fn picked() -> protocross.tests.TopLevelSta");
    ...
    {
        var link = await OneAsync("extend protocross.tests.Out");
    ...
    public async Task ADeclarationInTheAskedDocumentKeepsTheUriTheClientSent()
    {
        ...
        documents.Open(spelled, "protocross", 1, Source);
