...
public class BackendTests
{
    ...
    public void CSharpEmitsExtensionMethodsOverGeneratedMessages()
    {
        ...
        Assert.Contains(
            "public static long LineTotalCents(this global::ProtoCross.Examples.InvoiceItem self)",
            source,
            StringComparison.Ordinal);
        Assert.Contains(
            "public static long TotalCents(this global::ProtoCross.Examples.Invoice self)",
            source,
            StringComparison.Ordinal);
    }
    ...
    public void CSharpStatesWrappingExplicitly()
    {
        ...
        Assert.Contains(
            "unchecked(total + global::ProtoCross.Examples.InvoiceItemProtoCrossExtensions.LineTotalCents(item))",
            source,
            StringComparison.Ordinal);
    }
    ...
    [Fact]
    public void CSharpEmitsXUnitTestsForProtoCrossUnitTests()
    {
        ...
        var files = new CSharpBackend().EmitTests(
            result.Module!,
            new BackendOptions("simpleScript.pcross"),
            diagnostics);
        ...
        Assert.Contains(
            "[global::Xunit.Fact(DisplayName = \"protocross.examples.Invoice.total_cents: sums line totals\")]",
            source,
            StringComparison.Ordinal);
        Assert.Contains("var receiver = new global::ProtoCross.Examples.Invoice", source, StringComparison.Ordinal);
        ...
        Assert.Contains(
            "global::Xunit.Assert.Equal(1100L, global::ProtoCross.Examples.InvoiceProtoCrossExtensions.TotalCents(receiver));",
            source,
            StringComparison.Ordinal);
    }

    [Fact]
    public void CppEmitsStandaloneTestsForProtoCrossUnitTests()
    {
        var result = Compilation.Compile(TestPaths.SimpleScript, [TestPaths.ExampleProtoDirectory]);
        Assert.True(result.Success, string.Join("\n", result.Diagnostics.Select(d => d.ToString())));
        ...
        Assert.Contains("#include \"simpleScript.pc.h\"", source, StringComparison.Ordinal);
        Assert.Contains("int main(int argc, char** argv)", source, StringComparison.Ordinal);
        Assert.Contains("::protocross::examples::Invoice receiver;", source, StringComparison.Ordinal);
        ...
        Assert.Contains(
            "const auto actual = ::protocross::examples::total_cents(receiver);",
            source,
            StringComparison.Ordinal);
        ...
        // something.
        Assert.Contains(
            "::std::cout << \"[ok] protocross.examples.Invoice.total_cents: sums line totals\"",
            source,
            StringComparison.Ordinal);
        Assert.Contains("::std::cout << \"protocross: 12 test(s), \"", source, StringComparison.Ordinal);
    }
    ...
    public void CSharpOmitsTestSupportWhenNoTestExpectsFailure()
    {
        ...
        var files = new CSharpBackend().EmitTests(
            result.Module!, new BackendOptions("simpleScript.pcross"), new DiagnosticBag());
    ...
    public void BothBackendsGenerateOutOfProcessTestsForExpectedFailure()
    {
        ...
        Assert.Contains(
            "global::ProtoCross.Runtime.ProtoCrossTestSupport.DescribeExpectFail(",
        ...
        Assert.Contains("protocross_expect_fail(argv[0]", cppSource, StringComparison.Ordinal);
        Assert.Contains("::std::system(command.c_str())", cppSource, StringComparison.Ordinal);
        Assert.Contains("return kProtoCrossDidNotTerminate;", cppSource, StringComparison.Ordinal);
    }
    ...
    {
        var source = EmitSingle(new CppBackend(), ".pc.h");

        // Bare 'a * b' on int64 would be undefined behavior on overflow.
        Assert.Contains(
            "::protocross_runtime::wrap_mul_i64(self.quantity(), self.unit_price_cents())",
            source,
            StringComparison.Ordinal);
    ...
    public void CppDeclaresEveryMethodBeforeDefiningAny()
    {
        ...
        const string LineTotalSignature =
            "inline ::std::int64_t line_total_cents(const ::protocross::examples::InvoiceItem& self)";
        const string TotalSignature =
            "inline ::std::int64_t total_cents(const ::protocross::examples::Invoice& self)";
    ...
    public void CppIncludesTheGeneratedProtobufHeader()
    {
        ...
        Assert.Contains("#include \"protocross_runtime.h\"", source, StringComparison.Ordinal);
    }
    ...
    public void BothBackendsRouteOnZeroFailThroughTheFailHelper()
    {
        ...
        var cpp = Emit(new CppBackend(), path, out var cppDiagnostics)
            .Single(f => f.RelativePath.EndsWith("test.pc.h", StringComparison.Ordinal)).Contents;
    ...
    {
        var source = EmitSingle(new CppBackend(), "simpleScript.pc.h");
