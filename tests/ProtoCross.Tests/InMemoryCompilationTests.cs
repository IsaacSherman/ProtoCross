...
public class InMemoryCompilationTests
{
    ...
    {
        var identity = UnwrittenFile("buffer.pcross");

        var result = Compile(
            identity,
            Prelude + "extend Outer { fn f() -> int64 { return no_such_field; } }");

        var diagnostic = Assert.Single(result.Diagnostics, d => d.Code == "PC0037");
        Assert.Equal("buffer.pcross", diagnostic.Span.File);
    }
    ...
    {
        var path = TestPaths.WriteTempScript("@@@ this is not protocross @@@");
    ...
    public void AnUnsavedBufferInsideAProjectStillFindsItsConfig()
    {
        ...
        var result = Compilation.Compile(
            new SourceDocument(SourceIdentity.Unsaved("draft.pcross", directory), Prelude + Body),
            Fixtures);
    ...
    public void AnUnsavedBufferWithNoImportsIsToldSo()
    {
        ...
        Assert.Contains(result.Diagnostics, d => d.Code == "PC0001");
    }
    ...
    public void AMalformedIncludePathDoesNotHideSyntaxDiagnostics()
    {
        ...
        Assert.False(result.Success);
        Assert.Contains(result.Diagnostics, d => d.Code == "PC0010" || d.Code == "PC0011");
    }
    ...
    public void AMalformedIncludePathIsADiagnosticNotACrash()
    {
        ...
        Assert.False(result.Success);
        var diagnostic = Assert.Single(result.Diagnostics, d => d.Code == "PC0082");
    ...
    public void AMalformedIncludePathDoesNotAlsoBlameTheImport()
    {
        ...
        Assert.DoesNotContain(result.Diagnostics, d => d.Code == "PC0002");
    ...
    private static SourceIdentity UnwrittenFile(string name = "buffer.pcross")
