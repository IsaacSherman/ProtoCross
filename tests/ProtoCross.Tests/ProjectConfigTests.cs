...
public class ProjectConfigTests
{
    ...
    {
        var directory = Path.Combine(Path.GetTempPath(), "protocross-config", Guid.NewGuid().ToString("N"));
    ...
    public void ValuesAreTrimmedButCaseSensitive()
    {
        ...
        Assert.Contains(lowercasedDiagnostics, d => d.Code == "PC2002");
    }
    ...
    public void AnUnknownSectionIsRejected()
    {
        ...
        Assert.Null(config);
        var diagnostic = Assert.Single(diagnostics, d => d.Code == "PC2001");
    ...
    public void AnUnknownSettingIsRejected()
    {
        ...
        Assert.Null(config);
        Assert.Contains(diagnostics, d => d.Code == "PC2001");
    }
    ...
    public void AnUnknownValueIsRejectedAndTheLegalOnesAreNamed()
    {
        ...
        Assert.Null(config);
        var diagnostic = Assert.Single(diagnostics, d => d.Code == "PC2002");
    ...
    public void ADuplicateSettingIsRejected()
    {
        ...
        Assert.Null(config);
        Assert.Contains(diagnostics, d => d.Code == "PC2004");
    }

    [Fact]
    public void MalformedXmlIsRejectedWithItsLocation()
    {
        var (config, diagnostics) = Load("<ProtoCross><Arithmetic></ProtoCross>");

        Assert.Null(config);
        var diagnostic = Assert.Single(diagnostics, d => d.Code == "PC2003");
    ...
    public void AWrongRootElementIsRejected()
    {
        ...
        Assert.Null(config);
        Assert.Contains(diagnostics, d => d.Code == "PC2003");
    }
    ...
    public void BothBackendsDescribeTheBuildIdentically()
    {
        ...
        var cpp = EmitHeader(new CppBackend(), OverflowPolicy.Saturating, "test.pc.h");
    ...
    public void BackendOptionsDescribeTheDefaultPolicyWhenNothingSaysOtherwise()
    {
        Assert.Equal(
            ...
            new BackendOptions("test.pcross").PolicyDescription);
    }
    ...
    public void ABadConfigFileStopsTheCompilation()
    {
        ...
        Assert.Contains(result.Diagnostics, d => d.Code == "PC2002");
    }
    ...
    public void AConfigDiagnosticCarriesAnOffsetThatAgreesWithItsLineAndColumn()
    {
        ...
        var span = Assert.Single(diagnostics, d => d.Code == "PC2001").Span;
