...
using Xunit;
using DiagnosticSeverity = ProtoCross.LanguageServer.Protocol.Lsp.DiagnosticSeverity;
using WorkspaceFolder = ProtoCross.LanguageServer.Workspace.WorkspaceFolder;
...
public class WorkspaceTrustTests
{
        ...
        WorkspaceTrust trust)
    {
        ...
        return (configuration, DocumentUri.FromPath(Path.Combine(directory, "source.pcross")));
    }
    ...
    private static (WorkspaceConfiguration Configuration, DocumentUri Document) Stating(
        ...
        WorkspaceTrust trust)
    {
        var (configuration, document) = Folder(scope is ConfigurationSource.FolderSetting ? settings : ProtoCrossSettings.None, trust);
    ...
    public void AnUntrustedWorkspaceDoesNotUseAProtocItsOwnSettingsName(ConfigurationSource scope)
    {
        ...
        var (configuration, document) = Stating(scope, new ProtoCrossSettings { ProtocPath = protoc }, WorkspaceTrust.Untrusted);
    ...
    {
        var protoc = FileNamedProtoc();
        var (configuration, document) = Folder(new ProtoCrossSettings { ProtocPath = protoc }, WorkspaceTrust.NotReported);
    ...
    public void AProtocNamedAtUserScopeIsUsedInAnUntrustedWorkspace()
    {
        ...
        var (configuration, document) = Stating(
            ConfigurationSource.UserSetting, new ProtoCrossSettings { ProtocPath = protoc }, WorkspaceTrust.Untrusted);
    ...
    public void AWithheldProtocFallsThroughToTheNextSourceInTheOrdinaryOrder()
    {
        ...
        var (configuration, document) = Folder(new ProtoCrossSettings { ProtocPath = committed }, WorkspaceTrust.Untrusted);
    ...
    public void SettingsThatDoNotRequireTrustAreUsedInAnUntrustedWorkspace()
    {
        ...
        File.WriteAllText(policy, "<ProtoCross></ProtoCross>");

        var settings = new ProtoCrossSettings { IncludePaths = [schemas], ConfigPath = policy, ProtocPath = FileNamedProtoc() };
    ...
    {
        var (configuration, document) = Folder(new ProtoCrossSettings { ProtocPath = FileNamedProtoc() }, WorkspaceTrust.Untrusted);
    ...
    {
        var protoc = FileNamedProtoc();
        var (configuration, document) = Folder(new ProtoCrossSettings { ProtocPath = protoc }, WorkspaceTrust.Untrusted);

        var withheld = Assert.Single(configuration.Resolve(document).Withheld);

        Assert.Equal(ProtoCrossSettings.ProtocPathKey, withheld.Key);
    ...
    {
        var (configuration, document) = Folder(new ProtoCrossSettings { ProtocPath = FileNamedProtoc() }, WorkspaceTrust.Trusted);
    ...
    {
        var first = WorkspaceFolder.FromPath(TestPaths.CreateTempDirectory(), settings: new ProtoCrossSettings { ProtocPath = FileNamedProtoc() });
        var second = WorkspaceFolder.FromPath(TestPaths.CreateTempDirectory(), settings: new ProtoCrossSettings { ProtocPath = FileNamedProtoc() });

        var configuration = WorkspaceConfiguration.Empty with
        {
            Folders = [first, second],
            Workspace = new ProtoCrossSettings { ProtocPath = FileNamedProtoc() },
            User = new ProtoCrossSettings { ProtocPath = FileNamedProtoc() },
    ...
    public void WithheldSettingsAreAnnouncedOnce()
    {
        ...
        Assert.Contains(ProtoCrossSettings.ProtocPathKey, announcement, StringComparison.Ordinal);
    ...
    /// <remarks>
    /// A sweep over what <see cref="ProtoCrossSettings.Read"/> actually accepts, rather than over the
    ...
    {
        var definitions = ProtoCrossSettings.Definitions;

        Assert.Equal(ProtoCrossSettings.Keys, definitions.Select(definition => definition.Key));
        ...
        foreach (var key in ProtoCrossSettings.Keys)
        {
            var diagnostics = new DiagnosticBag();
            ProtoCrossSettings.Read([new SettingValue(key, "value")], ConfigurationSource.FolderSetting, diagnostics);
    ...
    public void OnlyTheProtocPathRequiresTrust()
        => Assert.Equal([ProtoCrossSettings.ProtocPathKey], ProtoCrossSettings.RestrictedKeys);
    ...
    {
        var everything = ProtoCrossSettings.Read(
            ProtoCrossSettings.Keys.Select(key => new SettingValue(key, $"stated for {key}")),
            ConfigurationSource.FolderSetting,
            new DiagnosticBag());

        var kept = everything.WithoutRestricted(out var withheld);

        foreach (var definition in ProtoCrossSettings.Definitions)
        {
    ...
    public async Task ACommittedProtocDoesNotRunBeforeTrustIsGranted()
    {
        ...
        var uri = new Uri(Path.Combine(directory, "source.pcross")).AbsoluteUri;
    ...
    public async Task AWithheldProtocIsAnnouncedToTheUser()
    {
        ...
        Assert.Contains(ProtoCrossSettings.ProtocPathKey, shown.Message, StringComparison.Ordinal);
    ...
    public async Task TheStatusReportNamesWhatTrustIsWithholding()
    {
        ...
        Assert.Equal(ProtoCrossSettings.RestrictedKeys, Facts(status, "requires trust").Select(fact => fact.Value));
