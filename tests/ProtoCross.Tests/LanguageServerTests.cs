...
{
    /// <summary>A file with no imports: it compiles as far as PC0001 and never reaches protoc.</summary>
    /// <remarks>
    ...
    private static string WriteDocument(string source, string name = "source.pcross")
    {
    ...
    public async Task ARequestBeforeInitializeIsRefusedRatherThanAnswered()
    {
        ...
        var refusal = await client.RefusalAsync(
            ...
            new SemanticTokensParams { TextDocument = new TextDocumentIdentifier { Uri = "file:///x.pcross" } });
    ...
    public async Task TheThreeNewRequestsAreAnsweredOverTheWire()
    {
        ...
        var path = Path.Combine(EditorFixture.DirectoryWithSchemas(), "source.pcross");
    ...
    public async Task SignatureParameterLabelsRemainStringsUnlessOffsetsWereNegotiated()
    {
        ...
        var uri = UriOf(Path.Combine(EditorFixture.DirectoryWithSchemas(), "source.pcross"));
        client.Notify(Methods.DidOpen, Open(uri, Bound));
    ...
    public async Task OpeningADocumentPublishesDiagnosticsForIt()
    {
        ...
        Assert.Contains(published.Diagnostics, diagnostic => diagnostic.Code == "PC0001");
    }
    ...
    public async Task EditingABufferUpdatesDiagnosticsWithoutSaving()
    {
        ...
        var updated = await client.DiagnosticsAsync(
            uri,
            published => published.Diagnostics.Any(diagnostic => diagnostic.Code != "PC0001"));
    ...
    public async Task ADiagnosticThatSpansSeveralLinesKeepsBothOfItsEnds()
    {
        ...
        var wholeFile = Assert.Single(published.Diagnostics, diagnostic => diagnostic.Code == "PC0001");

        // PC0001 is about the compilation unit, which is every line of it. A range collapsed to its
    ...
    public async Task TheDiagnosticCodeAndHelpTextBothSurvive()
    {
        ...
        var published = await client.DiagnosticsAsync(uri, message => message.Diagnostics.Count > 0);
        var noImports = Assert.Single(published.Diagnostics, diagnostic => diagnostic.Code == "PC0001");

        // The title, which LSP has no field for and which spec 26's own template puts first. Losing it
        // costs most on PC0003, whose message is whatever protoc wrote and which then says nothing
        // about who is speaking.
        Assert.StartsWith("no proto imports:", noImports.Message, StringComparison.Ordinal);

        // PC0001's help says what to write instead. This client declared relatedInformation, so the
    ...
    public async Task AnAnswerThatWentStaleWhileItWasComputedIsRefusedRatherThanPublished()
    {
        ...
        var uri = ProtoCross.LanguageServer.Workspace.DocumentUri.FromPath(WriteDocument(NoImports));
    ...
    public async Task ADiagnosticThatIsResolvedIsCleared()
    {
        ...
        // Replacing the whole buffer with something whose only complaint is PC0001.
        ...
        var fixedUp = await client.DiagnosticsAsync(
            uri,
            message => message.Version == 2 && message.Diagnostics.All(diagnostic => diagnostic.Code == "PC0001"));
    ...
    public async Task ASettingTheServerDoesNotUnderstandIsReportedRatherThanDropped()
    {
        ...
        var published = await client.DiagnosticsAsync(
            uri,
            message => message.Diagnostics.Any(diagnostic => diagnostic.Code == "PC2102"));

        var ignored = Assert.Single(published.Diagnostics, diagnostic => diagnostic.Code == "PC2102");
    ...
    public async Task ASettingThatStatesLanguagePolicyIsRefusedAndSaysWherePolicyLives()
    {
        ...
        var published = await client.DiagnosticsAsync(
            uri,
            message => message.Diagnostics.Any(diagnostic => diagnostic.Code == "PC2101"));

        var refused = Assert.Single(published.Diagnostics, diagnostic => diagnostic.Code == "PC2101");

        Assert.Contains("protocross.config.xml", refused.Message, StringComparison.Ordinal);
    }
    ...
    public async Task ADiagnosticWithNoLocationIsPublishedAtTheStartOfTheDocument()
    {
        ...
        var settingsWarning = Assert.Single(published.Diagnostics, diagnostic => diagnostic.Code == "PC2102");
    ...
    public async Task ASettingChangeTakesEffectWithoutARestart()
    {
        ...
        Assert.DoesNotContain(before.Diagnostics, diagnostic => diagnostic.Code == "PC2102");
        ...
        Assert.Contains(after.Diagnostics, diagnostic => diagnostic.Code == "PC2102");
    }
    ...
    public async Task DiagnosticsFromASupersededVersionAreNeverPublished()
    {
        ...
        Assert.All(
            settled.Diagnostics,
            diagnostic => Assert.Equal("PC0001", diagnostic.Code));
    ...
    public async Task AProtocErrorIsPublishedAgainstTheProtoItNamesAndSummarizedOnTheImport()
    {
        ...
        var document = Path.Combine(directory, "source.pcross");
        ...
        // PC0003 reprints the whole of standard error, so it is replaced rather than published beside
        // the per-line diagnostics parsed out of that same text.
        Assert.DoesNotContain(onTheDocument.Diagnostics, diagnostic => diagnostic.Code == "PC0003");
    }
    ...
    public async Task AProtoKeepsItsDiagnosticsWhileAnotherOpenDocumentStillReportsThem()
    {
        ...
        var first = Path.Combine(directory, "first.pcross");
        var second = Path.Combine(directory, "second.pcross");
    ...
    public async Task AProtocThatWasNamedAndIsNotThereIsReportedRatherThanReplaced()
    {
        ...
        var published = await client.DiagnosticsAsync(
            uri,
            message => message.Diagnostics.Any(diagnostic => diagnostic.Code == "PC2105"));

        var reported = Assert.Single(published.Diagnostics, diagnostic => diagnostic.Code == "PC2105");
