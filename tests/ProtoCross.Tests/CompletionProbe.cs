    ...
    CompilationResult Result)
{
    ...
    /// with <c>has</c> accepted over <c>count</c> is <c>has * scale</c>, which is <c>PC0080</c> for
    /// the same reason <c>var</c> accepted there is a syntax error. The sweep has always tolerated the
    /// syntax errors, and recognizing <c>PC0080</c> made one keyword out of five fail for a reason
...
internal static class CompletionProbe
{
    ...
    /// (<c>PC0059</c>, <c>PC0068</c>), and an operand of <c>has</c> that is not a field at all
    /// (<c>PC0080</c>). Codes for contexts not yet built are here from the start, because the cost of
    ...
    /// sweep would have applied it, recompiled, seen <c>PC0058</c> and not recognized the code. And
    /// <c>PC0080</c> was missing while <c>has</c> had no rule of its own, so the sweep watched every
    ...
    /// <remarks>
    /// <para>
    /// <c>PC0078</c> and <c>PC0079</c> are both about a field that resolved perfectly well, and both
    ...
    /// </para>
    /// <para>
    /// The line between them and <c>PC0080</c> above is whether the name could ever have been right
    ...
    /// </para>
    /// </remarks>
    public static readonly IReadOnlyList<string> AboutTheValueRatherThanTheName = ["PC0078", "PC0079"];
    ...
    /// <c>protocross.tests.Outer</c> is one name, an item there replaces the whole of it, and nothing
        ...
        SemanticModel written, string text, int caret, IReadOnlyList<AppliedItem> here)
    {
        ...
        // qualified name -- after the dot of 'protocross.' and before the line holding 'tests.Outer'
