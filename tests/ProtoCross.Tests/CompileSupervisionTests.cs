...
public class CompileSupervisionTests
{
    ...
    public async Task ASupersededCompileDoesNotPublishEvenWhenItFinishesLast()
    {
        ...
        documents.Open(uri, "protocross", version: 1, "extend Invoice {}");
        ...
        documents.Open(uri, "protocross", version: 2, "extend Invoice { }");
        scheduler.Schedule(uri);
    ...
    public async Task ManyDocumentsEditedAtOnceNeverExceedTheConcurrencyLimit()
    {
        ...
        {
            var uri = DocumentUri.FromPath(Path.Combine(directory, $"source-{index}.pcross"));
            documents.Open(uri, "protocross", version: 1, "extend Invoice {}");
            scheduler.Schedule(uri);
        }
    ...
    public async Task TypingIntoOneDocumentNeverQueuesMoreThanOneCompileForIt()
    {
        ...
        {
            documents.Open(uri, "protocross", version, $"extend Invoice {{ {new string(' ', version)} }}");
            scheduler.Schedule(uri);
    ...
    public async Task SchedulingACompileReturnsBeforeTheCompileRunsHoweverShortTheInterval()
    {
        ...
        var documents = new DocumentStore();
        documents.Open(uri, "protocross", version: 1, Source);
    ...
    public void ARequestHoldingADocumentHoldsTheVersionItRead()
    {
        ...
        var read = documents.Open(uri, "protocross", version: 1, "extend Invoice {}");
    ...
    {
        if (Environment.GetEnvironmentVariable("PROTOCROSS_SOAK") is not { Length: > 0 })
        {
            Assert.Skip("Set PROTOCROSS_SOAK=1 to run the long soak. Its short sibling runs every time.");
        }
    ...
    private static async Task SoakAsync(int documents, int edits)
    {
        ...
        var uris = Enumerable
            .Range(0, documents)
            .Select(index => DocumentUri.FromPath(Path.Combine(directory, $"source-{index}.pcross")))
            .ToList();
        ...
        {
            store.Open(uri, "protocross", version: 1, Source);
            scheduler.Schedule(uri);
        }
        ...
        for (var edit = 0; edit < edits; edit++)
        {
            ...
            store.Open(uri, "protocross", edit + 2, Source + new string(' ', edit % 8));
            scheduler.Schedule(uri);
        }
        ...
        int concurrency = CompileScheduler.DefaultConcurrency)
    {
        ...
        // PC2107 without a toolchain, which is all a test about scheduling needs of it.
