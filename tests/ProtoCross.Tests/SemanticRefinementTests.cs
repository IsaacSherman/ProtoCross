...
public class SemanticRefinementTests
{
    ...
    public async Task AQualifiedNameColoursEveryPartOfItselfAndNoneOfTheDots()
    {
        const string Qualified =
            ...
            extend protocross.tests.Outer {
                fn state() -> protocross.tests.TopLevelStatus {
                    return protocross.tests.TopLevelStatus.TOP_LEVEL_STATUS_OK;
                }
            }
            """;
        ...
        var extending = painted
            .Where(token => token.Line == 2 && token.TextIn(Qualified) is "protocross" or "tests" or "Outer")
            .ToList();
        ...
        var enumerated = painted
            .Where(token => token.Line == 3 && token.TextIn(Qualified) is "protocross" or "tests" or "TopLevelStatus")
            .ToList();
    ...
    public async Task ABrokenFileKeepsEveryTokenAndStillRefinesWhatBound()
    {
        ...
        Assert.Equal(
            PaintedToken.Decode(SemanticTokenEncoder.Encode(Broken, "source.pcross").Data).Count,
    ...
    public async Task AFileWhoseSchemaIsMissingIsStillClassifiedLexically()
    {
        ...
        Assert.Equal(
            SemanticTokenEncoder.Encode(Unloadable, "source.pcross").Data,
    ...
    public async Task ACategoryTheClientCannotPaintFallsBackToTheLexicalAnswer()
    {
        ...
        Assert.Equal(
            SemanticTokenEncoder.Encode(Source, "source.pcross").Data.Count,
