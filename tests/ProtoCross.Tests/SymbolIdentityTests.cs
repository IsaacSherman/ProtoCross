...
public class SymbolIdentityTests
{
    ...
    /// <c>buffer.pcross</c> for both of these, because that is what a diagnostic has to say; the
    ...
    /// <summary>
    /// The awkward case the issue names: a schema member has no ProtoCross declaration to point at,
    ...
    public void AnEnumValueIsIdentifiedByTheEnumThatDeclaresIt()
    {
        ...
        Assert.Equal(
            $"protocross.tests.ambiguous.First.Kind.{value.Name}",
    ...
    public void AnInnerBlockCannotShadowAnOuterLocal()
    {
        ...
        Assert.Contains(result.Diagnostics, d => d.Code == "PC0029");
    }
