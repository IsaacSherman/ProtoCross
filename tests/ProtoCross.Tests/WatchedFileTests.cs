...
/// <c>protocross.config.xml</c>. Saving one moves what the open documents say, with no keystroke in them.
/// </summary>
/// <remarks>
...
public class WatchedFileTests
{
    ...
    public async Task AClientThatCannotWatchIsNotAskedTo()
    {
        ...
        await client.RequestAsync(
            ...
            new DocumentSymbolParams { TextDocument = new TextDocumentIdentifier { Uri = "file:///nothing.pcross" } });
    ...
    public async Task RepairingAPolicyFileRepublishesTheDocumentItGoverns()
    {
        ...
        Assert.Contains(refused.Diagnostics, diagnostic => diagnostic.Code == "PC2106");

        File.WriteAllText(policy, "<?xml version=\"1.0\" encoding=\"utf-8\"?><ProtoCross></ProtoCross>");
        client.Notify(Methods.DidChangeWatchedFiles, Changed(policy));

        await client.DiagnosticsAsync(uri, published => published.Diagnostics.All(diagnostic => diagnostic.Code != "PC2106"));
    }
