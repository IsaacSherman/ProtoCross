    ...
    string? BinaryDirectory)
{
    ...
    public string DescribeMissingLinkInputs()
    {
        ...
        return $"The protobuf C++ headers at '{IncludeDirectory}' have no {string.Join(", ", missing)} "
            + "beside them. A vcpkg install provides all of them; set PROTOCROSS_PROTOBUF_CPP_INCLUDE "
...
internal static class Toolchain
{
    ...
    /// Stands in for <c>PROTOCROSS_PROTOBUF_CPP_INCLUDE</c>. A parameter rather than a variable the
    ...
    public static ProtobufCppInstall? LocateProtobufCpp(string? configuredInclude = null)
    {
        var configured = configuredInclude
            ?? Environment.GetEnvironmentVariable("PROTOCROSS_PROTOBUF_CPP_INCLUDE");
        ...
        params string[] protoFiles)
    {
        ...
        // google/protobuf/*.proto compiles in ProtoCross and then fails here, generating the message
