...
public class DocumentSymbolTests
{
    private const string Source =
        ...
        extend protocross.tests.Mapped {
            fn tallied() -> int64 {
                return count;
            }
        }
    ...
    {
        var extend = Named(Outline(Source), "protocross.tests.Outer");
    ...
    public void AMethodShowsTheSignatureAsItWasWritten()
        => Assert.Equal(
            "(scale: int64) -> int64",
            Named(Named(Outline(Source), "protocross.tests.Outer").Children!, "scaled").Detail);
    ...
    {
        var test = Named(
            Named(Outline(Source), "protocross.tests.Outer").Children!, "doubles what it is given");

        Assert.Equal(SymbolKind.Function, test.Kind);
        Assert.Equal("protocross.tests.Outer.scaled", test.Detail);
    }
    ...
    public void ATestWhoseReceiverNamesNoExtendBlockIsListedOnItsOwn()
    {
        var source = Source.Replace(
            "test protocross.tests.Outer.scaled", "test Outer.scaled", StringComparison.Ordinal);

        var outline = Outline(source);

        Assert.Equal(SymbolKind.Function, Named(outline, "doubles what it is given").Kind);
        Assert.DoesNotContain(
            Named(outline, "protocross.tests.Outer").Children!,
    ...
    public void TopLevelDeclarationsAppearInSourceOrder()
    {
        ...
        Assert.Equal(
            ["protocross.tests.Outer", "doubles what it is given", "protocross.tests.Mapped"],
    ...
    public void ATestWrittenAwayFromItsBlockIsListedOnItsOwn()
    {
        ...
        Assert.Equal(
            ["protocross.tests.Outer", "protocross.tests.Mapped", "doubles what it is given"],
    ...
    {
        var method = Named(Named(Outline(Source), "protocross.tests.Outer").Children!, "scaled");
    ...
    {
        var extend = Named(Outline(Broken), "protocross.tests.Outer");
    ...
    public void ADeclarationWithNoNameYetIsNamedForItsKeyword()
        => Assert.Contains(
            Named(Outline(Unnamed), "protocross.tests.Outer").Children!, child => child.Name == "fn");
    ...
    {
        var flattened = DocumentOutline.Flattened(Outline(Source), "file:///source.pcross");

        Assert.Equal(
            "protocross.tests.Outer",
            Assert.Single(flattened, symbol => symbol.Name == "scaled").ContainerName);

        Assert.Null(
            Assert.Single(flattened, symbol => symbol.Name == "protocross.tests.Outer").ContainerName);
    }
    ...
    public void AFlattenedOutlineHoldsEveryEntryTheTreeHeld()
        => Assert.Equal(
            ...
            DocumentOutline.Flattened(Outline(Source), "file:///source.pcross").Count);

    [Fact]
    public void EveryFlattenedEntryNamesTheDocumentItIsIn()
        => Assert.All(
            DocumentOutline.Flattened(Outline(Source), "file:///source.pcross"),
            symbol => Assert.Equal("file:///source.pcross", symbol.Location.Uri));
