...
public class MeasurementFailureTests
{
    ...
    public void AFailedCompilationCannotBeAcceptedAsAWarmWorkspace(string failureKind)
    {
        ...
        {
            File.WriteAllText(
                Path.Combine(Path.GetDirectoryName(workspace.Path)!, "protocross.config.xml"),
                "<protocross>");
        }
        else
        {
    ...
    {
        var previousBench = Environment.GetEnvironmentVariable("PROTOCROSS_BENCH");
        ...
        try
        {
            Environment.SetEnvironmentVariable("PROTOCROSS_BENCH", "1");
        ...
        finally
        {
            ...
            stop?.Invoke();
            Environment.SetEnvironmentVariable("PROTOCROSS_BENCH", previousBench);
