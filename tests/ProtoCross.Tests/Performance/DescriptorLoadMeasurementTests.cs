...
public class DescriptorLoadMeasurementTests
{
    ...
    public void ADescriptorLoadIsMeasuredColdAndWarm()
    {
        ...
        {
            Assert.Skip("Set PROTOCROSS_BENCH=1 to measure descriptor loads. See docs/performance.md.");
        }
    ...
    public async Task ConcurrentColdLoadsAreMeasuredAgainstTheConcurrencyLimit()
    {
        if (!Sampler.Requested)
        {
            Assert.Skip("Set PROTOCROSS_BENCH=1 to measure concurrent loads. See docs/performance.md.");
        }

        var protoc = RequireProtoc();
        var report = new PerformanceReport();
        var limit = ProtoCross.LanguageServer.Hosting.CompileScheduler.DefaultConcurrency;
        ...
        report.Note(
            $"The compile concurrency limit is {limit} and the answer limit is "
                + $"{ProtoCross.LanguageServer.Hosting.DeferredAnswers.DefaultConcurrency}.");
    ...
    public void TheDescriptorCacheFootprintIsAStatedNumber()
    {
        if (!Sampler.Requested)
        {
            Assert.Skip("Set PROTOCROSS_BENCH=1 to measure the cache footprint. See docs/performance.md.");
        }
