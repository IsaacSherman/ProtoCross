...
public class SchemaCompletionTests
{
    ...
    private static (CompletionProvider Provider, DocumentStore Documents, DocumentUri Uri) Open(string text)
    {
        ...
        var path = Path.Combine(TestPaths.CreateTempDirectory(), "source.pcross");
    ...
    private static (CompletionProvider Provider, DocumentUri Uri, string Text) Beside(string body)
    {
        ...
        var name = $"source{Interlocked.Increment(ref _sources)}.pcross";
    ...
    /// <summary>
    /// A method named without being called is PC0040, so the parentheses are part of the item rather
    ...
    /// alone is PC0037, since BindName never looks at methods. The parentheses are what make the item
    /// one that binds.
    /// </summary>
    [Fact]
    ...
    public async Task ATypePositionOffersTheImportedMessagesAndEnumsUnderBothSpellings()
    {
        ...
        Assert.Contains("protocross.tests.Outer", Labels(offered));
        Assert.Contains("TopLevelStatus", Labels(offered));
        Assert.Contains("protocross.tests.TopLevelStatus", Labels(offered));
    }
    ...
    public async Task ATypePositionOffersTypesNestedInsideAMessage()
    {
        ...
        Assert.Contains("Inner", Labels(offered));
        Assert.Contains("protocross.tests.Outer.Inner", Labels(offered));
        Assert.Contains("protocross.tests.Outer.Nested", Labels(offered));
    }

    /// <summary>
    /// Two packages declaring one simple name make it PC0074 to write unqualified, so offering it
    ...
    public async Task AnAmbiguousSimpleTypeNameIsOfferedOnlyInItsQualifiedForms()
    {
        ...
        Assert.Contains("protocross.tests.ambiguous.First.Kind", Labels(offered));
        Assert.Contains("protocross.tests.ambiguous.Second.Kind", Labels(offered));

        var qualified = Assert.Single(
            offered, item => item.Label == "protocross.tests.ambiguous.First.Kind");
    ...
    /// <summary>
    /// ResolveMessage never looks at enums, so one offered here would be PC0021 the moment it was
    /// accepted.
    /// </summary>
    [Fact]
    ...
    public async Task AMessageWhoseSimpleNameAnEnumSharesIsStillOfferedAfterExtend()
    {
        ...
        Assert.Contains("protocross.tests.shared.Shape", Labels(offered));
    }

    /// <summary>
    /// The other half of the same fixture, and the other answer. In a type position messages and
    /// enums share one name space, so the very name that is unambiguous as a receiver is PC0074 here
    ...
    public async Task TheSameNameIsAmbiguousInATypePositionAndNotAsAReceiver()
    {
        ...
        Assert.Contains("protocross.tests.shared.Shape", Labels(asAType));
        Assert.Contains("protocross.tests.shared.Holder.Shape", Labels(asAType));
    ...
    /// <summary>A singular field written twice is PC0061, so one already given a value is spent.</summary>
    [Fact]
    ...
    /// Accepting a method here produces PC0080 rather than an ordinary return-type mismatch.
    /// </summary>
    [Fact]
    ...
    public async Task CompletionInAResolvedPresenceCheckNeverOffersAMethodResult()
    {
        ...
        {
            Assert.True(
                !attempt.About.Any(diagnostic => diagnostic.Code == "PC0080"),
                $"accepting '{attempt.Item.Label}' as a presence operand produced PC0080; "
    ...
    /// <summary>
    /// PC0080 says it outright: a local, a parameter and a method result always hold a value, so
    ...
    /// name that produces PC0058 the moment it is accepted -- and accepting it strands a method the
    ...
    /// <summary>An argument supplied twice is PC0065, so one already written is spent.</summary>
    [Fact]
    ...
    /// local offered at 'x.field' produces PC0039 the moment it is accepted.
    /// </summary>
    [Fact]
    ...
    /// expression keywords, and each one produces <c>PC0039</c> the moment it is accepted -- because
    ...
    {
        const string body = "extend Outer {\n    fn f() -> int64 {\n"
            + "        var local: protocross.tests.Outer.Inner = inner;\n        return count;\n    }\n}\n";

        var (provider, uri, text) = Beside(body);
        var name = text.IndexOf("protocross.tests.Outer.Inner", StringComparison.Ordinal);
        ...
        var offered = (await provider.AnswerAsync(asked!, CancellationToken.None)).Items;
        var lines = new LineMap(text);
        var end = lines.PositionOf(name + "protocross.tests.Outer.Inner".Length);
    ...
    public async Task AnEnumValuedFieldDoesNotOfferConstantsAsInstanceMembers()
    {
        ...
        Assert.True(
            offered.Count == 0,
            "status is an enum-valued field; every status.CONSTANT edit would fail with PC0039");
    }
    ...
    {
        const string name = "protocross.tests.Outer";
    ...
    public async Task AFixtureValueExpressionDoesNotOfferReceiverFieldNames()
    {
        ...
        Assert.True(
            ...
            "a field offered after '=' becomes an unresolved value name (PC0037), not a fixture field");
    }
    ...
    [Theory]
    [InlineData(
        "extend Outer { fn f(other: protocross.\n tests.Outer) -> int64 { return count; } }",
        "tests.Out")]
    [InlineData(
    ...
    /// <c>protocross.tests.pcross.tests.Outer</c> and a sibling composes a name nothing declares.
    ...
    public async Task RetainedNamePartsKeepQualifiedTypesCompletable(string body, string marker)
    {
        ...
        var uri = DocumentUri.Parse(new Uri(Path.Combine(root, "qualified.pcross")).AbsoluteUri);
        var documents = new DocumentStore();
        documents.Open(uri, "protocross", 1, text);
    ...
    [Theory]
    [InlineData("extend protocross. /* receiver */\n tests.Outer { fn f() -> int64 { return count; } }")]
    [InlineData("extend Outer { fn f(other: protocross. // parameter\n tests.Outer) -> int64 { return count; } }")]
    [Trait("ReviewRegression", "SchemaCompletionQualification")]
    ...
    public async Task TheLastSegmentOfAQualifiedTypeStillOffersWholeTypeNames()
    {
        var (provider, uri, text) = Beside(
            "extend Outer { fn f(other: Outer) -> int64 {\n"
                + "var local: protocross.tests.Outer = other; return count; } }");

        var applied = await CompletionProbe.SweepAsync(
            provider, uri, text, [After(text, "var local: protocross.tests.Out")], uri.Path!, Loader());
    ...
    {
        if (Environment.GetEnvironmentVariable("PROTOCROSS_SWEEP") is not { Length: > 0 })
        {
            Assert.Skip("Set PROTOCROSS_SWEEP=1 to sweep the whole corpus. Its bounded siblings run every time.");
        }

        var source = Assert.Single(CompiledCorpus.All, candidate => candidate.Name == name);
        var documents = new DocumentStore();
        var path = Path.Combine(EverySchema.Value, $"corpus-{name}.pcross");
        var uri = DocumentUri.Parse(new Uri(path).AbsoluteUri);

        documents.Open(uri, "protocross", 1, source.Text);
