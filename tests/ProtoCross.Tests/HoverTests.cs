...
public class HoverTests
{
    private const string Source =
        ...
        extend protocross.tests.Outer {
            ...
            fn describes() -> bool {
                var picked: protocross.tests.TopLevelStatus = TopLevelStatus.TOP_LEVEL_STATUS_OK;
    ...
    private static string Fenced(string signature) => $"```protocross\n{signature}\n```";
    ...
    public async Task ALoopBindingShowsTheTypeOfOneElement()
    {
        ...
        Assert.Contains(
            Fenced("value: protocross.tests.Outer.Nested"),
    ...
    public async Task AMethodShowsItsWholeSignature()
        => Assert.Contains(
            "fn scaled(scale: int64) -> int64",
            await TextAfterAsync("test protocross.tests.Outer.scal"),
            StringComparison.Ordinal);

    [Fact]
    public async Task AMethodSaysWhichMessageItIsAttachedTo()
        => Assert.Contains(
            "Method on `protocross.tests.Outer`.",
            await TextAfterAsync("test protocross.tests.Outer.scal"),
            StringComparison.Ordinal);
    ...
    public async Task AnEnumValueShowsItsEnumType()
        => Assert.Contains(
            "TOP_LEVEL_STATUS_OK: protocross.tests.TopLevelStatus",
    ...
    public async Task AMessageTypeShowsTheFullNameEvenWhereASimpleOneWasWritten()
    {
        var source = Source.Replace(
            "var picked: protocross.tests.TopLevelStatus",
    ...
    {
        var card = await TextAfterAsync("extend protocross.tests.Out");

        Assert.Contains("protocross.tests.Outer", card, StringComparison.Ordinal);
    ...
    /// <summary>
    /// The arithmetic policy is a choice <c>protocross.config.xml</c> made that the source text does
    ...
    public async Task TheOverflowPolicyStatedIsTheOneTheProjectConfigured(string policy, string expected)
    {
        ...
        File.WriteAllText(
            ...
            $"<ProtoCross><Arithmetic><Overflow>{policy}</Overflow></Arithmetic></ProtoCross>");
    ...
    {
        var card = await CardAsync(Source, EditorFixture.After(Source, "extend protocross.tests.Out"));
        var start = EditorFixture.At(Source, "protocross.tests.Outer {");

        Assert.NotNull(card);
        Assert.NotNull(card!.Range);
        Assert.Equal(
            EditorPositions.Between(new Diagnostics.LineMap(Source), start, start + "protocross.tests.Outer".Length),
            card.Range);
    }
