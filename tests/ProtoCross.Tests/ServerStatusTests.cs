...
public class ServerStatusTests
{
    ...
    /// not start. Pointing <c>protocross.protocPath</c> at the downloaded archive, or at the directory
    ...
    public void ASettingNamingSomethingThatIsNotProtocIsReportedWithTheReasonItFailed()
    {
        ...
        var document = DocumentUri.FromPath(Path.Combine(EditorFixture.DirectoryWithSchemas(), "source.pcross"));
    ...
    public void EveryResolvedSettingIsReportedWithTheLayerThatSuppliedIt()
    {
        ...
        {
            ["protocross"] = new Dictionary<string, object?> { ["includePaths"] = new[] { include } },
        }));

        var status = Reporter(configuration: configuration)
            .Report(DocumentUri.FromPath(Path.Combine(directory, "source.pcross")));
    ...
    public async Task TheStatusRequestIsAnsweredBeforeTheServerIsInitialized()
    {
        ...
        Assert.Contains("ProtoCross language server status", status.Markdown, StringComparison.Ordinal);
    }
    ...
    public async Task TheExtensionVersionIsWhateverTheClientSaidItWas()
    {
        ...
        var told = await client.RequestAsync(
            ...
            {
                ExtensionName = "protocross-vscode",
    ...
    private static JsonElement Settings(string protocPath)
        ...
        {
            ["protocross"] = new Dictionary<string, object?> { ["protocPath"] = protocPath },
        });
}
