...
public class NameMappingTests
{
    ...
    private static (IReadOnlyList<MessageDescriptor> Messages, string Header) GenerateCppHeader()
    {
        ...
        File.WriteAllText(Path.Combine(directory, AwkwardFieldSchemaName), AwkwardFieldSchema);

        var generated = Toolchain.RunProtoc(protoc, "cpp_out", directory, directory, AwkwardFieldSchemaName);
        ...
        var schema = new DescriptorLoader(protoc)
            .LoadBundle([AwkwardFieldSchemaName], [directory])
            .Descriptors.Single(file => file.Name == AwkwardFieldSchemaName);

        return ([.. schema.MessageTypes], File.ReadAllText(Path.Combine(directory, "awkward_fields.pb.h")));
    }
