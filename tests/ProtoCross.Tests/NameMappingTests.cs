...
public class NameMappingTests
{
        ...
        string generatedName)
    {
        ...
        File.WriteAllText(Path.Combine(directory, schemaName), schema);

        var generated = Toolchain.RunProtoc(protoc, outputFlag, directory, directory, schemaName);
        Assert.True(generated.ExitCode == 0, $"protoc --{outputFlag} failed.{Environment.NewLine}{generated.Output}");

        var file = new DescriptorLoader(protoc)
            .LoadBundle([schemaName], [directory])
            .Descriptors.Single(descriptor => descriptor.Name == schemaName);

        return ([.. file.MessageTypes], File.ReadAllText(Path.Combine(directory, generatedName)));
    }
}
