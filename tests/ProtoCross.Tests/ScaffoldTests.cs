...
public class ScaffoldTests
{
    ...
    /// <summary>
    /// The three include roots the generated driver needs. It includes its ProtoCross header
    ...
    {
        // cross_caller.proto imports cross_target.proto, which the ProtoCross source never names.
    ...
    public void TheEmittedCMakeProjectIgnoresNonCppSources()
    {
        var options = CppOptions() with
        {
            TestSourceFileNames = ["simpleScript.tests.cc", "ProtoCrossTestSupport.g.cs"],
        };

        var cmake = CppTestProject.Build(options);

        Assert.Contains("add_executable(simpleScript_tests ", cmake, StringComparison.Ordinal);
        Assert.DoesNotContain("ProtoCrossTestSupport", cmake, StringComparison.Ordinal);
    }
}
