...
public class DocumentSemanticsTests
{
    ...
    private static WorkspaceConfiguration Settings(string root)
        ...
        {
            Generation = 1,
            User = new ProtoCrossSettings { IncludePaths = [root] },
        };
    ...
    /// types.proto. Only that dependency changes, leaving both the ProtoCross buffer and every file
    ...
    private static void AssertDependencyRepairIsVisible(string declaration)
    {
        ...
        var uri = DocumentUri.Parse(new Uri(Path.Combine(root, "unchanged.pcross")).AbsoluteUri);
        ...
        Assert.Contains(failed.Result.Diagnostics, diagnostic => diagnostic.Code == "PC0003");
    ...
    [Theory]
    [InlineData(null, "PC0002")]
    [InlineData("syntax = \"proto3\"; message Counter {", "PC0003")]
    ...
    public void AWellKnownSchemaFromAnImplicitRootDoesNotRecompileAnUnchangedDocument()
    {
        ...
        var root = TestPaths.CreateTempDirectory();
        var uri = DocumentUri.Parse(new Uri(Path.Combine(root, "well-known.pcross")).AbsoluteUri);
        var document = new DocumentStore().Open(
            uri, "protocross", 1,
    ...
    {
        var root = TestPaths.CreateTempDirectory();
        var policy = Path.Combine(root, "protocross.config.xml");
        File.WriteAllText(policy, "<not-valid-xml");

        var uri = DocumentUri.Parse(new Uri(Path.Combine(root, "unchanged.pcross")).AbsoluteUri);
        var document = new DocumentStore().Open(uri, "protocross", 1, Source);
        ...
        File.WriteAllText(policy, "<ProtoCross />");
    ...
    public void ACompilationFinishingAfterCloseCannotRestoreTheForgottenDocument()
    {
        ...
        var uri = Uri("closing.pcross");
        var document = documents.Open(uri, "protocross", 1, Source);
    ...
    {
        var (semantics, documents, configuration) = Fresh();
        var uri = Uri("edited.pcross");

        var before = documents.Open(uri, "protocross", 1, Source);
    ...
    {
        var (semantics, documents, configuration) = Fresh();
        var uri = Uri("reopened.pcross");

        var first = documents.Open(uri, "protocross", 1, Source);
        semantics.For(first, configuration, CancellationToken.None);

        documents.Close(uri);
        var second = documents.Open(uri, "protocross", 1, Source);
    ...
    public void WhatIsHeldNeverGrowsPastTheNumberOfDocumentsAskedAbout()
    {
        ...
        {
            documentsAsked.Add(documents.Open(Uri($"many{index}.pcross"), "protocross", 1, Source));
        }
    ...
    public void ADocumentWhoseConfigurationWasRefusedIsRememberedRatherThanRetried()
    {
        ...
        File.WriteAllText(Path.Combine(directory, "protocross.config.xml"), "<not-valid-xml");

        var documents = new DocumentStore();
        var semantics = new DocumentSemantics(Loaders());
        var configuration = Settings(TestPaths.FixtureProtoDirectory);
        var uri = DocumentUri.Parse(new Uri(Path.Combine(directory, "refused.pcross")).AbsoluteUri);
        var document = documents.Open(uri, "protocross", 1, Source);
    ...
    public async Task TheCompileTheSchedulerRanIsTheOneEverythingElseAsksAbout()
    {
        ...
        var path = Path.Combine(TestPaths.CreateTempDirectory(), "shared.pcross");
    ...
    public async Task ClosingADocumentLetsGoOfWhatWasCompiledForIt()
    {
        ...
        var path = Path.Combine(TestPaths.CreateTempDirectory(), "transient.pcross");
        ...
        client.Notify(
            ...
            new DidOpenTextDocumentParams
            {
                TextDocument = new TextDocumentItem
                {
                    Uri = uri, LanguageId = "protocross", Version = 1, Text = text,
                },
            });
