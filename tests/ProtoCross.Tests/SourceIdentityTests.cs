...
public class SourceIdentityTests
{
    ...
    {
        var written = Path.Combine("a", "b", "buffer.pcross");

        Assert.Equal(Path.GetFileName(written), SourceIdentity.FromPath(written).Name);
        Assert.Equal("buffer.pcross", SourceIdentity.FromPath(written).Name);
    }

    [Fact]
    public void ExpandsTheDirectoryEvenWhenThePathIsRelative()
    {
        var identity = SourceIdentity.FromPath(Path.Combine("a", "b", "buffer.pcross"));
    ...
    public void AnUnsavedBufferCanStillBelongToADirectory()
    {
        ...
        var identity = SourceIdentity.Unsaved("draft.pcross", directory);

        Assert.Null(identity.Path);
        Assert.Equal("draft.pcross", identity.Name);
