...
- **A `protoc` that could not be found at all is reported in the editor's words, not the command
  line's.** `PC0003` keeps its code, its title and its place, and its message says that no `protoc`
  was found, where the host looked -- `PROTOCROSS_PROTOC`, `PATH` and the NuGet package cache, root by
  root -- where protoc is published, and that it can then go on `PATH` or be named by
  `protocross.protocPath`. The command line's message is unchanged: it is written for somebody building
...
- **A compilation whose schemas failed to load is not reused at all**, which is 21.1's rule applied
  ...
  failure until they edit the ProtoCross buffer -- which is the one thing they have no reason to do
