...
- Operands of a binary arithmetic operator must already have the same type. ProtoCross applies no
...
- Backends **must** emit each arithmetic operation explicitly rather than relying on the target
  ...
  property of the consumer's build; ProtoCross semantics must be a property of the generated code.
...
`Arithmetic/Overflow` in `protocross.config.xml` ([10.4](#104-compile-time-policy)) selects one of three answers. Every mode is
...
- Integer `/` and `%` are a compile error (`PC0054`) without an `on_zero` clause, unless the divisor
...
This is also why ProtoCross does not need `Result` in order to ship. `Result` remains the better
...
- `on_zero fail` gives generated library code the ability to terminate the host process. That is
  ...
  server embedding ProtoCross behavior has no way to opt out.
...
`Arithmetic/Conversion` in `protocross.config.xml` ([10.4](#104-compile-time-policy)) names this behavior. It has one legal
...
`protocross.config.xml`, next to the code they govern.
...
- **A host that explains an operation states the policy governing it, and states it nowhere else.**
  The header serves a reader of the generated code; a reader of the ProtoCross source has no such
...
and which of them are C#'s own behavior rather than something ProtoCross invented.
...
`protocross.config.xml`. Left to each client to answer, that becomes two settings models and a server
...
- **A configuration file that is found and cannot be read stops the document, and says so.**
  `PC2106` is an error, not a warning, and names the file, how it came to be consulted, every problem
...
- **Trust is the client's to report and the host's to apply.** A client states it as a boolean
  ...
  `protocross/didChangeWorkspaceTrust` notification, whose `trusted` member says which way it changed;
...
- **A client starts the host so that nothing the host looks up can resolve into the workspace.** A
  bare `protoc`, a relative `PATH` entry and a relative `PROTOCROSS_PROTOC` or `NUGET_PACKAGES` all
  ...
  `PROTOCROSS_PROTOC` and `NUGET_PACKAGES` passed on only when absolute. Absolute `PATH` entries are kept,
...
- `WorkspaceConfiguration` in `ProtoCross.LanguageServer` is the model, and `Resolve` is the only
