...
- ProtoCross does not define an independent application type system.
- ProtoCross value types are protobuf scalar primitives, protobuf enums, and protobuf messages.
- ProtoCross does not add non-protobuf numeric types such as `decimal`.
...
ProtoCross accepts all protobuf scalar spellings that name a value domain. Wire-encoding variants
collapse to the decoded value type:

| Protobuf spelling | ProtoCross type |
|---|---|
| `double` | `double` |
| `float` | `float` |
...
- The spelling may be used in a ProtoCross type reference when protobuf defines it.
- Once decoded, `sint32`, `sfixed32`, and `int32` have the same ProtoCross type; likewise for the
...
- ProtoCross does not support additional value types outside the protobuf type universe.
- `decimal` is not supported because Protocol Buffers do not define a decimal scalar type.
- Backends must not silently map a ProtoCross type to target-specific types such as C# `decimal`, Python `Decimal`, or arbitrary-precision numeric classes unless that value is represented by an explicit protobuf message type.

Implementation Note:

- Projects that need decimal-like behavior should define an explicit protobuf message, such as a fixed-scale money or decimal representation, and then define ProtoCross behavior over that message.
...
repeated fields. ProtoCross does not re-derive those rules; it asks the descriptor.
...
- Its operand must name a protobuf field. A local, a parameter, a literal, and a method result
  always hold a value, so there is no question to ask about them (`PC0080`).
...
- A field admits the question exactly when protobuf says it has presence. That is one rule covering
  ...
  schema.
- `has` on a field with no presence is `PC0079`. It is not `false`: the field has no answer, and
